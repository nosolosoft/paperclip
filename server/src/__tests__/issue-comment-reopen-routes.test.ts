...
describe.sequential("issue comment reopen routes", () => {
  ...
  it("wakes the current assignee when an assigned issue moves to in_review", async () => {
    const issue = makeIssue("blocked");
    mockIssueService.getById.mockResolvedValue(issue);
    mockIssueService.update.mockImplementation(async (_id: string, patch: Record<string, unknown>) => ({
      ...issue,
  ...
  it("does not duplicate the assignment wake when reassignment also moves an issue to in_review", async () => {
    const issue = makeIssue("blocked");
    mockIssueService.getById.mockResolvedValue(issue);
    mockIssueService.update.mockImplementation(async (_id: string, patch: Record<string, unknown>) => ({
      ...issue,
  ...
  it("does not wake on in_review when no agent is assigned", async () => {
    const issue = {
      ...makeIssue("blocked"),
      assigneeAgentId: null,
      assigneeUserId: "local-board",
